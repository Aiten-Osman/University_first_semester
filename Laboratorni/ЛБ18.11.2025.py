...
#създаване на приложение с модули 
# !!!принципи на обектно ориентирано програмиране!!! важно
# стринг метод


# създайте обектно ориентирана система, която управлява автомобили за отдаване под наем.
# Използвайте класове, наследяване-> super(), енкапсулация и добра стуктурираност на кода.
# Искаме базов клас Car, който трябва да има:
# id, marka, model, godina, bazova cena na den, probeg, tip gorivo, neobhodima 
# kategoriq shofiorska knijka, dali e nalichen ili ne
# Този клас трябва да има методи за изчисляване (override)на наема на ден, маркиран
#  като наличен или неналичен, представяне на информацията на автомобила
# Подкласове на Car: икономична кола - 10% отстъпка от базовата цена, 
# Premium Car - 50% над базовата цена,sqv - 30% увеличение, електрическа кола - 20% отстъпка
...
# Клас Reservation: автомобил, клиент, брой дни, крайна цена, статус 
# (дали резервацията е active, pending, completed, cancelled)
...
# за резервации да създава с проверка за валидност, активиране, приключване и отказ;
#  извеждане на активни и приключени резервации
# за отчет - общ приход, клиенти с активни наеми
class Car:
    def __init__(self, id, marka, model, year, price_per_day, probeg, type_fuel, licence, available):
        self.id = id
        self.marka = marka
        self.model = model
        self.year = year
    ...
    def rent_per_day(self):
        pass #->тва се override 

    def is_available(self):
        if self.available:
            return "Yes, the car is available."
        else:   
            return "No, the car is not available."

    def __str__(self): #-> магически метод за принтиране на обект/ стринг представяне на обект
    ...
    # /n за нов ред при принтиране на обект
...
economy_car = EconomyCar(1, 'BMW', 'X1', 2007, 40, 10000, 'fuel1', 'licence1', True)
premium_car = EconomyCar(2, 'Mercedes', 'C220', 2010, 50, 20000, 'fuel2', 'licence2', False)
sqv_car = EconomyCar(3, 'Opel', 'Mokka', 2017, 60, 30000, 'fuel3', 'licence3', True)
electric_car = ElectricCar(4, 'VW', 'Tiguan', 2023, 45, 25000, 'fuel4', 'licence4', False)
...
customer1 = Customer('Aiten', '0888888888', 'aitenosman80@gmail.com', 'licence1', [])
